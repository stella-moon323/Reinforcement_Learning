...
class Agent:
    def __init__(self, config: Config, logger):
        ...
        # モデル定義
        self.online_net = QNet(self.num_observations, self.num_actions, config.model.hidden_num, config.model.noisy_sigma)
    ...
    def train_model(self):
        ...
        next_pred = self.online_net(next_states).squeeze(1)
        target = rewards + masks * self.config.agent.gamma * next_pred.max(1)[0]
